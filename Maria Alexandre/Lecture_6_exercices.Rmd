...
```{r}
coef(firstreg) #extraire vecteur des coeff
```
...
```{r}
seconreg<- lm(Ozone ~ Wind, data=airquality)
summary(seconreg)
plot(seconreg)
coef(seconreg)
```
...
lm(Ozone ~ Temp, data=airquality) %>%
  augment() %>%
  ggplot(aes(y=Ozone, x=Temp))+
  geom_point(color="purple")+
  geom_line(aes(x= Temp, y= .fitted),color= "red")+
  geom_segment(aes(x=Temp, xend=Temp, y= .fitted, yend= Ozone), color="grey")+
  theme_minimal()
```
```
...
df%>%
group_by(continent)%>%
summarise(corr=cor.test(.$lifeExp, .$gdpPercap)%>% tidy())
```
...
Est ce que le gdp per capita est inférieur à 2000 par continent et par année
