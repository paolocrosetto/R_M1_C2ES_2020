...
# étape 1:créer  les variables d'intérêt
...
#étape2 :créer le plot en version brute
#il faut que toutes les dimensions soient au bon endroit

base%>%
  ggplot(aes(x=incident_month,y=incident_year,fill=n))+
  facet_wrap((~operator))+
  geom_tile(col="white")->plot_base
plot_base
# étape3 :
#rendre le plot 'beau'-> plus similaire à la cible
...
scale_fill_gradient(low="white",high="#ee3d8e")+
  theme_minimal()+
  theme(panel.background = element_rect(fill="grey90"),plot.background =     
  element_rect(fill="grey90"),plot.title = element_text(hjust=0.5))+
          labs(title="Airlines Birds Strikes",x="Month",y="Year")
...
#stratégie 
#étape1 :données(summarise, mutate)

#étape 2 graphique brut(ggplot,facet,geom)

#étape3 graphique final(scale,theme)
```
