...
lm(Ozone~Wind,data=airquality)%>%
  ...
  geom_point()+
  geom_errorbar(aes(xmin=estimate-std.error,xmax=estimate+std.error,height=0.1))
```


#exercice
regression de Ozone sur Wind et Temp +plot des coefficients
```{r}
#stratégie
# 1. faire le lm()
...
lm(Ozone~Temp,data=airquality)%>%
  ...
  geom_point()+
  geom_line(aes(x=Temp,y=.fitted),col="red",size=2)+
  theme_minimal()
...
lm(formula=lifeExp~gdpPercap,data=df)%>%
  ...
  geom_point()+
  geom_line(aes(x=gdpPercap,y=.fitted),col="red")
```
...
df%>%
 ...
 # lm(formula=lifeExp~gdpPercap,data=.)==>ça ne marche pas
  summarise(n=n())
# .表示 group_by(continent,year)的数据
```
...
reg_result%>%
  ...
  ggplot(aes(x=year,y=estimate,col=continent))+
  geom_point()+
...
```{r}
cor.test(df$lifeExp,df$gdpPercap)->correclation #%>%tidy()
```
...
df%>%
  group_by(continent)%>%
  summarise(corr=cor.test(.$lifeExp,.$gdpPercap))
```
...
# 2. appliquer group_modify et cor_test +tidy
# 3. filter les pays qu'il nous faut
...
df%>%
  ...
  ggplot(aes(year,x,col=continent))+
  geom_point()+
  facet_wrap(~continent)
...
> est-ce que le gdp percapita est inférieur à 2000€/an par continent
