...
df_speed %>%
  ...
  summarise(df,mean1=mean(speed_km_h,na.rm=TRUE))
...
##quelle compagnie aéroenne va plus vite
df%>%
...
df%>%
  ...
  mutate(speed=distance/air_time)%>%
  group_by(carrier)%>%
  summarise(maxspeed=mean(speed,na.rm=TRUE),meandist=mean(distance,na.rm=TRUE))%>%
  arrange(-maxspeed)
