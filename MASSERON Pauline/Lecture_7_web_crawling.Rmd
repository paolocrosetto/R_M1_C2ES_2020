...
africa %>% 
  ...
  rename(nation = Nation, gdp_agri = 'GDP from agriculture (%)') %>% 
  mutate(gdp_agri = as.numeric(gdp_agri)) %>% 
  ggplot(aes(x = reorder(nation,gdp_agri), y = gdp_agri)) +
  geom_col() +
  coord_flip()
...
#date
date <- imbd %>% html_nodes(".secondaryInfo") %>% html_text()
  
#étoiles
stars <- imdb %>% html_nodes(".imdbRating") %>% html_text()

#coller les vecteurs ensemble
...
Rcrawler(Website = "https://www.newgrounds.com/games", 
         crawlUrlfilter = "/portal/view/", dataUrlfilter = "/portal/view/")
