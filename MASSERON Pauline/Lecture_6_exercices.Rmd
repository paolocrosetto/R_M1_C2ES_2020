...
Régression de Ozone sur Wind et Temps + plot des coefficients

```{r}
lm(Ozone ~ Wind + Temp, data = airquality) %>% 
  tidy() %>% 
  ggplot(aes(x = estimate,y = term)) + geom_point(size = 5)
...
df %>% 
  group_by(continent) %>% 
  summarise(~corr = cor(.$lifeExp, .$gdpPercap) %>% tidy())
```
...
est-ce que le gdp est inférieur à 2000 euros par an par continent par année ? 
