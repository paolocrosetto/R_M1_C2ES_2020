...
lm(formula=Ozone~Wind,data=airquality)%>%
  tidy()%>%
  ggplot(aes(y=term,x=estimate))+geom_point()+geom_errorbar(aes(xmin=estimate-std.error,xmax=estimate+std.error),height=0.1)+theme_minimal()

```

##exercice
regretion de ozone sur wind et temp(airquality)+plot des coeffiences
```{r}
#strategie
#1,faire le lm
...
#3,faire le plot()
lm(formula = Ozone~Wind+Temp,data=airquality)%>%tidy()%>%ggplot(aes(y=term,x=estimate))+
  geom_point(size=3,pch=12)
...
```{r}
lm(Ozone~Temp,data = airquality)%>%augment()%>%ggplot(aes(x=Temp,y=Ozone))+geom_point()+geom_line(aes(x=Temp,y=.fitted),color="green")+theme_minimal()
...
```{r}
lm(Ozone~Temp,data = airquality)%>%augment()%>%ggplot(aes(x=Temp,y=Ozone))+geom_point()+geom_line(aes(x=Temp,y=.fitted),color="green")+geom_segment(aes(x=Temp,xend=Temp,y=.fitted,yend=Ozone) )+theme_minimal()
```
...
```{r}
lm(Ozone~Temp,data=airquality)%>%glance()
...
```{r}
lm(lifeExp~gdpPercap,data = df)%>%tidy()
```

cela à l'air très significatif. On va faire un plot en utilisant `augment` pour vérifier de façon visuelle

```{r}
lm(lifeExp~gdpPercap,data = df)%>%augment()%>%ggplot(aes(x=gdpPercap,y=lifeExp))+geom_point()+geom_line(aes(x=gdpPercap,y=.fitted),col="red")
```
...
```{r}
df%>%group_by(continent,year)%>%lm(lifeExp~gdpPercap,data = .)#.explique les data which is in groupby
##
df%>%group_by(continent,year)%>%summarise(n=n())
...
```{r}
df%>%group_by(continent,year)%>%group_modify(~ lm(lifeExp~gdpPercap,data = .)%>%tidy)->reg_resultat
```
...
```{r}
reg_resultat%>%filter(term!="(Intercept)")%>%
                        ggplot(aes(x=year,y=estimate,color=continent))+geom_point()+facet_wrap(~continent)+geom_hline(yintercept = 0,col="red")
```
...
```{r}
cor.test(df$lifeExp,df$gdpPercap)%>%tidy()
```
...
```{r}
df%>%summarise(corr=cor(df$lifeExp,df$gdpPercap))

##
df%>%group_by(continent)%>%group_modify(~cor.test(df$lifeExp,df$gdpPercap))
```
...
```{r}
df%>%group_by(continent)%>%group_modify(~cor.test(df$lifeExp,df$gdpPercap)%>%tidy())
```
...
> calculez l'interval de confiance de la corrélation entre lifeExp et gdpPercap pour chaque continent pour chaque année. Plottez les résultats pour la France, le Japon et la Bolivie au fil du temps. 

```{r}
#1,grouper par country and year
#2, appliquer group_modify et cor.test and tidy
#3,filter les pays qu'ilnous faut
#4,faire le plot des interveaux de confiance
df%>%
group_by(continent,year)%>%
  filter(is.na(lifeExp)==FALSE,is.na(gdpPercap)==FALSE)%>%
  group_modify(~cor(df$lifeExp,df$gdpPercap)%>%tidy)%>%ggplot(aes(year,x,col=continent))+geom_point()+facet_wrap(~continent)
```
...
df%>%
group_by(continent,year)%>%
  group_modify(~t.test(.$lifeExp)%>%tidy)%>%ggplot(aes(x=year,y=estimate,color=continent))+facet_wrap((~continent))+geom_point()+geom_hline(yintercept = 60,col="red")+geom_errorbar(aes())
...
#>est ce que le gdp percapita est inferieur a 2000$par an continent pR ANNEE?
  #startegie
  #1,group by
  #2. t.test(group_modify,tidy)
  #3,t.test(var,mu=k,alternative="greater"?"smaller")
  #4,plot
  df%>%group_by(continent,year)%>%
  group_modify(~t.test(.$gdpPercap,mu=2000)%>%tidy)%>%
  ggplot(aes(x=continent,y=estimate,col=year))+
  facet_wrap(~year)+
  geom_point()+
