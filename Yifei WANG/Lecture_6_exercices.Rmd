...
```
> Faites une régression de Ozone sur Temp et Wind 
...
```{r}
df %>%
   group_by(continent, year) %>% # Marche pas 
   # summarise(n = n())
...
df %>%
   ...
   group_modify(~ lm(pop ~ year, data = .) %>% tidy) -> reg_result
```
...
```{r}
cor.test(df$lifeExp, df$gdpPercap) -> correlation
...
> calculez l'interval de confiance de la corrélation entre lifeExp et gdpPercap pour chaque continents pour chaque année. Plottez les résultats pour la France, le Japon et la Bolivie au fil du temps. 

```{r}
# Strategie
# Grouper par country et year
# Appliquer group_modify() et cor.test() + tidy
# Filter les pays qu'il nous faut
# Faire le plot des intervalles de confiances
df %>%
  group_by(continent, year) %>%
...
> est-ce que le dgp percapita est inferieur a 2000 euros/an par continent par annee?
...
# t.test(df$gdpPercap, mu = 2000, alternative = "less")
