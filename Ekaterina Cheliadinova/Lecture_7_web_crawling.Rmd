...
# LIRE LA PAGE
# read_html(URL)
# html_node -> une fois selectionnes les nodes avec un CSS selector
...
read_html('https://en.wikipedia.org/wiki/List_of_African_countries_by_GDP_(PPP)_per_capita') %>% 
  ...
  .[[1]] %>% 
  html_table(fill = T) %>% 
  rename(gdp_agr = 'GDP from agriculture (%)', nation = Nation) %>% 
  mutate(gdp_agr = as.numeric(gdp_agr)) %>% 
  ggplot(aes(y = reorder(nation, gdp_agr ), x = gdp_agr))+
  geom_col()
```
...
films <- read_html('https://www.imdb.com/chart/top/')
titre <- films %>% html_nodes('.titleColumn a') %>% html_text()
date <- films %>% html_nodes('.secondaryInfo') %>% html_text()
n_etoiles <- films %>% html_nodes('strong') %>% html_text()

data.frame(titre,date,n_etoiles) %>% 
  mutate(date = as.numeric(gsub('\\(|\\)','',date)), n_etoiles = as.numeric(n_etoiles))
