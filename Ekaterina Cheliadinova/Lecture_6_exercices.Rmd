...
firstreg <- lm(Ozone ~ Solar.R + Wind + Temp, data = airquality) # regression liniare
```
...
lm(Ozone ~ Temp, data = airquality) %>% 
  ...
  geom_point()+
  geom_line(aes(Temp, .fitted), color = 'red')+
...
```{r}
lm(Ozone ~ Temp, data = airquality) %>% 
  glance() # помогает определить лучшую модель 
```
...
```{r}
df %>%
  ggplot(aes( x = year, y = lifeExp, group = country))+ #spagetti plot
  geom_line() -> spaghetti
```
...
df %>% 
  ...
  lm(lifeExp~ gdpPercap, data = .) # создал только 1ну регрессию
```
...
```{r}
cor.test(df$lifeExp, df$gdpPercap) -> cortest
```
...
df %>% 
  group_by(continent, year) %>% 
  filter(is.na(lifeExp)==F,is.na(gdpPercap)==F) %>% 
  group_modify(~cor(.$lifeExp,.$gdpPercap) %>% tidy()) %>% 
...
> est-ce que le gdp percapita est inferieur a 2000 euro par an par continent par annee

```{r}

df %>% 
  group_by(continent, year) %>% 
  group_modify(~t.test(x = .$gdpPercap, mu = 2000, alternative = 'greater') %>% tidy()) %>% 
  ggplot(aes(y = estimate, x = continent, color = year))+
  facet_wrap(~year)+
  geom_point()
