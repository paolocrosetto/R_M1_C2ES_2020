...
# 1. lire la page
# 2. read_html(URL)
# 3. html_node(s) -> une fois sélectiooner les nodes
# 4. html_table() -> importer dans une data frame
```
...
```{r}
#read_html(URL)
#html_node(s) -> une fois sélectiooner les nodes
# html_table() -> importer dans une data frame
africa <- read_html("https://en.wikipedia.org/wiki/List_of_African_countries_by_GDP_(PPP)_per_capita")

africa %>% 
  html_node (".wikitable")-> nation
...
# création vecteur titre
titre_film <- film %>% html_nobes(".titleColumn a") %>% html_text()

#création vecteur date
date <- film %>% html_nobes(".secondaryInfo") %>% html_text()

#création vecteur étoiles
stars <- film %>% html_nobes(".imbRating") %>% html_text()

#coller les data
imb_data <- tibble(titre_film, date , stars )
View(imb_data)
