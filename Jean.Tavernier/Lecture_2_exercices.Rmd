...
df %>%
  ...
  select(origin, dest, dep_time) -> df_midday
...
```{r ex4}
réponse4 <- df %>% 
  mutate(impact = arr_delay/arr_time-dep_time * 100) %>% 
  select(impact, everything()) 
réponse4
...
```{r ex5}
réponse5 <- df %>% 
  summarise(mean = mean(arr_delay,na.rm = TRUE), sd = sd(arr_delay,na.rm = TRUE), min = min(arr_delay,na.rm = TRUE),max = max(arr_delay,na.rm = TRUE))
réponse5
```
