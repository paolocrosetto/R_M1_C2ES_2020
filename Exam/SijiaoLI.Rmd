...
###  Obtention des données originales
...
### 	Reconstruction des données-Sélecter les 8 variable

```{r}
new_table<-select(coffee_ratings,total_cup_points,species,country_of_origin,company,region,color,acidity,sweetness)
new_table
```
...
```{r}
df<-new_table%>%
  rename("notes"="total_cup_points","espèce"="species","pays"="country_of_origin",
           "nom_entreprise"="company","région"="region","couleur"="color",
         "acidité"="acidity","douceur"="sweetness")
df
```
...
##  Jeu des données et Graphiques

> Question 1 : Quel couleur de grain de café a la note le plus haute?

```{r}
note_haute_couleur<-df%>%
  select(notes,couleur)%>% group_by(couleur)%>%summarise(max(notes, na.rm = TRUE))
note_couleur<-df%>%
  select(notes,couleur)%>% group_by(couleur)%>%summarise(mean(notes, na.rm = TRUE))%>%
...
plot2<-plot1+facet_wrap(~couleur)
plot(plot2)
```
...
df%>%filter(espèce %in% c("Arabica","Robusta"))%>%
  select(espèce,notes)%>%group_by(espèce)%>%summarise(notemoyenne=mean(notes,na.rm = TRUE))%>% 
  ggplot(aes(x=espèce,y=notemoyenne))+
 ...
 df%>%filter(espèce %in% c("Arabica","Robusta"))%>%
  select(espèce,notes)%>%group_by(espèce)%>%ggplot(aes(x=espèce,y=notes))+
...
coord_equal(ratio = 1,
                    xlim = c(0, 10),
                    ylim = c(0, 100))+ scale_size_area(max_size = 30)+
  coord_fixed(ratio = 1/30)+
...
>>>>>>> 3b64026786a42ae9b63ab741c056d40d6b80150c
