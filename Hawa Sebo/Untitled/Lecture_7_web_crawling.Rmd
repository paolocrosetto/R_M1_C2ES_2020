...
```{r, message=FALSE}
usa %>% html_nodes(".wikitable") ->table
...
# lire la page
# read_html(URL)
# html_node(s) -> une fois selectionnés les nodes avec un CSS selector
# html_table() -> 
...
df %>% 
  ...
  #eliminer pourcentage
  separate(growth, into = c("growth", "sign_perc"), sep = -1) %>% 
  # growth est un caractere
...
# lire la page web
gdp <- read_html( "https://en.wikipedia.org/wiki/List_of_African_countries_by_GDP_(PPP)_per_capita ")

# chercher les odes qu'il nous faut
gdp %>% html_node(".wikitable") ->table_gdp
...
df_gdp %>% 
  ...
  rename(nation = Nation, GDP ="GDP from agriculture (%)") %>% 
   mutate(GDP = as.numeric(GDP)) %>% 
  ggplot(aes(x =reorder(nation,GDP), y = GDP))+
  geom_col()+
  coord_flip()
...
## le titre l'anée et le nombre d'étoile du film

# lire la page web

film <- read_html("https://www.imdb.com/chart/top/")

# cre&tion vecteur titre

titres_film <- film %>% html_nodes(".titleColumn a") %>% html_text()

# cre&tion vecteur date
date_film <- film %>% html_nodes(".secondaryInfo") %>% html_text()

# cre&tion vecteur etoile

nombre_etoile <- film %>% html_nodes("strong") %>% html_text()

data_film <- tibble(titres_film, date_film, nombre_etoile)
```
...
Rcrawler(Website = "https://www.newgrounds.com/games",
         ...
         ExtractCSSPat = c("#sidestats > .sidestats:nth-child(1)   dd:nth-child(2)                                 ","#score_number ", ".rated-e"),
         PatternsNames = c("view", "score", "title"))
