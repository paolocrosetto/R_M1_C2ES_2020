...
plot(lm(Ozone ~ Wind, data = airquality))
...
regression de ozone sur Wind et Temp(airquality) + plot des
coefficients
...
#7.648826e-04 relation positive
# plus on est riche plus on a une esperance de vie plus élevée
```
...
#group_by ne marche pas car corr ne connait pas group_by plutot utiliser
#group_modify
```
...
> est-ce-que le gdp percapita est inferieur à 2000€ /an par continent
par année?
...
df %>% 
  ...
  group_modify(~t.test(var = .$gdpPercap, mu = 2000,
