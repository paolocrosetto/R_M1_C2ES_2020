...
il faut passer par une commande qu'on ne connait pas -- `group_modify`. C'est une fonction qui applique une fonction à chaque groupe. C'est un peu comme un `summarize()`. Avec la différence que `summarize` retourne **une** valeur pour chaque groupe (example: moyenne) alors que `group_modify` accepte toute fonction qui retourne un data.frame. Prenez `group_modify` comme un `summarize` on steroids. 
...
et voilà. On a obtenu un data.frame qui contient une estimation *par ligne* -- dans ce cas par année/continent. 
