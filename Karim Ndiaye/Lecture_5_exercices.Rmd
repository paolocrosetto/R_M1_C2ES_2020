...
#Etape1:créer les variables d'intéret
...
summarise(n=n()) %>% 
incident_month(as.character()->df2


##Etape2:créer le plot en version brute
...
##Etape3:rendre le plot beau -> plus similaire à la cible
# utiliser scale_ et them_

df_debase +
  coord_cartesian(ylim=c(2000,2018)) +
  scale_fill_gradient(low="white",high="#ee3d8e")+
  theme_minimal()+
  theme(panel.background = element_rect(fill="grey90"),plot.background = element_rect(fill="grey90"),color=)->dfde
...
## startegie
## etape1 données summarise(),mutate()
## etape2 graphe brut(ggplot,facet,geom)
## etape2 graphe (scale,them)
```
