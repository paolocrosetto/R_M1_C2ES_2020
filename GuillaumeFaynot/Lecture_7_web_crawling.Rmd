...
africa %>% html_nodes(".wikitable")
pays <- africa %>% html_node(".wikitable")
pays %>% 
  html_table() %>% 
  as_tibble() -> df2
...
df2 %>% 
  ...
  rename(pays = Nation, gdpAgri = `GDP from agriculture (%)`) %>% 
  #gdpAgri est un caractère
  mutate(gdpAgri = as.numeric(gdpAgri)) %>% 
  # barplot
  ggplot(aes(x = reorder(pays,gdpAgri), y = gdpAgri))+
  geom_col()+
  coord_flip()
...
#2. pour chaque élement chercher le node et appliquer html_text
...
```{r, message = FALSE}
#Cette fonction permet de télécharger les pages demandées grâce à crawlUrlfilter
