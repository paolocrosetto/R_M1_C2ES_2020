...
ggplot(data=gas_price, aes(x=year, y=gas_current, colour = "black"))+
  #geom_point(size=2)+
  geom_line()+
  geom_line(aes(y = gas_constant, colour="blue"))+
  geom_smooth(method="lm", aes(color="red"))+
  scale_color_identity(name = "Légende",
                    breaks = c("black", "blue", "red"),
                    labels = c("gas_current", "gas_constant", "droite de régression"),
                    guide = "legend")+
...
state_pop %>% 
  select(year,pop) %>% 
  group_by(year) %>% 
  summarise(somme = sum(pop)) %>% 
  filter(!is.na(somme)) -> PopAugmentation
ggplot(PopAugmentation, aes(x = year, y = somme))+
...
ggplot(PopAugmentationPerState[1:5,], aes(x="", y=diff, fill=state))+
  ...
  labs(title="Représentation des TOP 5 états ayant connus \n la plus grosse augmentation de population depuis 1904")
```
...
barplot(NbParcParState$NombreDeParcs)
```
